...
@login_required
def remove_candidate(request, exam_id,candidate_id):
        exam = get_object_or_404(Exam, id=exam_id)
        candidate = get_object_or_404(user_Profile, id=candidate_id)
        if candidate in exam.candidates.all():
           exam.candidates.remove(candidate)
           messages.success(request,f'{candidate.username} has been removed succsesfully')
