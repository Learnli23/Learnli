...
class FlaggedContentAdmin(admin.ModelAdmin):
    list_display = ('content_object','content_type','object_id', 'reason', 'reported_by', 'date_reported','is_approved_status',
                    'is_reviewed_status','is_escalated_status')
    ...
    def is_escalated_status(self, obj):
        if obj.escalated:
            return format_html('<span style="color: green;">&#10004;</span>')  # Green check mark for reviewede
        else:
            return format_html('<span style="color: red;">&#10006;</span>')  # Red cross mark for not reviewed
    is_escalated_status.short_description = 'escalated Status'
